...
public class ExpireActivity extends AppCompatActivity {
    ...
    private Button btn_free, btn_prime;
    private RadioGroup rdg_free, rdg_prime;

    private String payCode = "";

    private void initEvent() {
        btn_free.setOnClickListener(view -> {
            switch (rdg_free.getCheckedRadioButtonId()) {
                case R.id.rdb_free_one:
                    payCode = "5";
                    getPayment("Free membership for a month", payCode, "USD");
                    break;
                case R.id.rdb_free_three:
                    payCode = "12";
                    getPayment("Free membership for 3 months", payCode, "USD");
                    break;
                case R.id.rdb_free_six:
                    payCode = "20";
                    getPayment("Free membership for 6 months", payCode, "USD");
                    break;
                default:
                    break;
            }
    ...
    private void initView() {
        ...
        rdg_free = findViewById(R.id.rdg_free);
        rdg_prime = findViewById(R.id.rdg_prime);

        rdg_free.check(R.id.rdb_free_one);
    ...
    private void onSuccessPayment(String payCode) {
        ...
        HttpUtil.onHttpNoImageResponse(HttpUtil.url_payment, params, new HttpUtil.HttpUtilCallback() {
            @Override
            public void onEventCallBack(JSONObject obj, int ret) {
                dialog.dismiss();
                try {
                    AppUtil.showToast(ExpireActivity.this, obj.getString("result"));
            ...
            public void onEventInternetError(Exception e) {
                dialog.dismiss();
                AppUtil.showToast(ExpireActivity.this, e.getMessage());
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_PAYMENT) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                AppUtil.showToast(ExpireActivity.this, "Payment Cancelled.");
            } else if (resultCode == PaymentActivity.RESULT_EXTRAS_INVALID) {
                AppUtil.showToast(ExpireActivity.this, "Payment Invalid.");
            }
        }
    }
